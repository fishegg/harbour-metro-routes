...
bool RouteSearch::getallnumber(const int &s, const int &d, const QList<Station> &fullstationlist, const QVector<int> &station_index){
    ...
    if(station_temp.interchangeat(4) != -1)
        source_list.enqueue(station_temp.interchangeat(4));
    if(station_temp.interchangeat(5) != -1)
        source_list.enqueue(station_temp.interchangeat(5));
    ...
    if(station_temp.unpaidinterchangeat(4) != -1)
        source_list.enqueue(station_temp.unpaidinterchangeat(4));
    if(station_temp.unpaidinterchangeat(5) != -1)
        source_list.enqueue(station_temp.unpaidinterchangeat(5));
    ...
    if(station_temp.interchangeat(4) != -1)
        destination_list.enqueue(station_temp.interchangeat(4));
    if(station_temp.interchangeat(5) != -1)
        destination_list.enqueue(station_temp.interchangeat(5));
    if(station_temp.unpaidinterchangeat(1) != -1)
    ...
    if(station_temp.unpaidinterchangeat(4) != -1)
        destination_list.enqueue(station_temp.unpaidinterchangeat(4));
    if(station_temp.unpaidinterchangeat(5) != -1)
        destination_list.enqueue(station_temp.unpaidinterchangeat(5));
    ...
    if(p == ConvenientlyTransfer){
        ...
        if(!use_temp_array){
            ...
            for(i=0; i<G->n(); i++){
                ...
                for(w=G->first(v); w<G->n(); w=G->next(v,w)){
                    ...
                    else if(G->weight(v,w) > 300 && G->weight(v,w) < 99999999){
                        replaced_weight = 100;
                    }
                    else{
                        replaced_weight = G->weight(v,w);
        ...
        if(!use_temp_array){
            ...
            for(i=0; i<G->n(); i++){
                ...
                for(w=G->first(v); w<G->n(); w=G->next(v,w)){
                    if(G->weight(v,w) < 200){
                        replaced_weight = 10000;
                    }
                    else if(G->weight(v,w) == 200 || G->weight(v,w) == 300){
                        replaced_weight = 20000;
                    }
                    else if(G->weight(v,w) > 300 && G->weight(v,w) < 99999999){
//                        replaced_weight = 500;
                        replaced_weight = G->weight(v,w);
        ...
        if(!use_temp_array){
            ...
            for(i=0; i<G->n(); i++){
                ...
                for(w=G->first(v); w<G->n(); w=G->next(v,w)){
                    ...
                    G->setmark(v, 1);
                    if(G->weight(v,w) < 300){
                        replaced_weight = 1500;
                    }
                    else if(G->weight(v,w) == 200 || G->weight(v,w) == 300){
                        replaced_weight = 3000;
                    }
                    else if(G->weight(v,w) > 300 && G->weight(v,w) < 99999999){
                        replaced_weight = G->weight(v,w);
//                        replaced_weight = 1500;
...
//    qDebug() << From;
    do
    {
        qDebug() << "into do while" << prev << From[prev];
        if(prev == From[prev])
        {
            break;
        }
